...
<li {{ $attributes->merge(['class' => 'group']) }}>
    ...
    <span class="absolute left-0 group-last:left-{{ $lastIconLeft }} group-odd:-top-12 group-even:top-21 flex items-center justify-center w-10 h-10 bg-mainLight dark:bg-mainDark rounded-full -start-5 ring hover:ring-2 ring-red">
        @if($work)
            <x-svgs.work />
        @else
            <x-svgs.school />
    ...
    <h3 class="capitalize text-base font-semibold mt-2 p-0"> {{ $slot }}</h3>
